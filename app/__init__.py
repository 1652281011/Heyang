...
def register_hooks(app):
    ...
    def before_request():
        """
        每次请求之前都会执行这个函数
        用于解析 Token 并设置 g.user
        """
        g.user = None
        g.auth_token = None
        
        # 1. 获取 Token (前端传的 Key 是 Auth-Token)
        ...
        if token:
            ...
            # 2. 【优先验证管理员】
            # 尝试用 Admin 模型解密 Token
            # verify_auth_token 内部应该处理了解密失败的情况并返回 None
            admin_user = Admin.verify_auth_token(token)
            if admin_user:
                g.user = admin_user
                g.user.role = 'admin'  # 标记身份为管理员
                return  # 验证成功，直接返回，不再往下走

            # 3. 【验证教授】
            # 如果不是管理员，尝试用 Professor 模型解密
            prof_user = Professor.verify_auth_token(token)
            if prof_user:
                g.user = prof_user
                g.user.role = 'professor' # 标记身份为教授
                return